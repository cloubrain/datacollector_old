			...
			for x2 in perfMetrics:
				try:
				 	# x2["perfkey"] = perfkeys[x2["k"]]
                                        my_value = local_dict.pop('perfMetrics', d=-1)
#					try:
#						del x["perfMetrics"]
#					except KeyError:
#						pass
	...
	def saveData(self, ts, filen):   # Get data and save to file
                """
                Why not accumulate all data for all clusters and save in an array?
                """
...
NULL		# 0: disk.commandsAborted.summation
NULL		# 1: mem.swapout.average 
NULL		# 2: disk.busResets.summation 
NULL		# 3: mem.swapped.average 
		# 4: mem.consumed.average 
		# 5: cpu.ready.summation
NULL		# 6: mem.swapin.average 
