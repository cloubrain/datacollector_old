...
def install():
  confFile = open("/etc/init/collect.conf", "w")
  ...
  output.close()
  confFile.write("exec python " + str(path) + "\n") 
...
def usage():
  ...
  print
  print "Options"
  print "\t-r, --run=PATH\t\t\tpath to store data collected from cloudstack"
